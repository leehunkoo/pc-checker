...
export default function App() {
  ...
  // autoPass 섹션 항목들 초기 pass 처리
  useEffect(()=>{
    const initResults={};
    SECTIONS.forEach(sec=>{
      if(sec.autoPass){
        sec.items.forEach(item=>{
          if(!item.autoKey) initResults[item.id]={status:"pass",detail:"수동 확인: 적합",autoChecked:false};
        });
      }
    });
    if(Object.keys(initResults).length>0) setResults(prev=>({...initResults,...prev}));
  },[]);
  ...
  const generateReport=async(format)=>{
    ...
    try{
      const res=await fetch("/api/report",{method:"POST",headers:{"Content-Type":"application/json"},
        body:JSON.stringify({results,format,inspector:{name:reportName,dept:reportDept,date:new Date().toLocaleDateString("ko-KR"),os:"Windows"}})});
  ...
  const CheckTab=()=>(
    <div style={{padding:"0 16px 120px"}}>
      ...
      {SECTIONS.map(sec=>{
        ...
        return <div key={sec.id} className="glass-card" style={{marginBottom:10,overflow:"hidden"}}>
          <div onClick={()=>setExpandedSection(isOpen?null:sec.id)} style={{padding:"16px",display:"flex",alignItems:"center",gap:12,cursor:"pointer"}}>
            <div style={{width:32,height:32,borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center",background:sec.bg,fontSize:16}}>{sec.emoji}</div>
            <div style={{flex:1}}>
              ...
              return <div key={item.id} className="check-row" style={{animationDelay:`${ii*0.06}s`}}>
                ...
                {showManualToggle&&<div style={{marginTop:10,paddingTop:10,borderTop:"1px solid rgba(255,255,255,0.06)",display:"flex",alignItems:"center",gap:10}}>
                  ...
                  {notes[item.id]!==undefined&&<input
                    ...
                    onBlur={e=>setNotes(n=>({...n,[item.id]:e.target.value}))}
        ...
        <div style={{marginBottom:8}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
            ...
            <input
              ...
              onCompositionEnd={e=>setReportName(e.currentTarget.value)}
              onBlur={e=>setReportName(e.target.value)}
              onChange={e=>{ if(!e.nativeEvent.isComposing) setReportName(e.target.value); }}
              placeholder="이름 입력"
          ...
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
            ...
            <input
              ...
              onCompositionEnd={e=>setReportDept(e.currentTarget.value)}
              onBlur={e=>setReportDept(e.target.value)}
              onChange={e=>{ if(!e.nativeEvent.isComposing) setReportDept(e.target.value); }}
              placeholder="부서 입력"
              style={{flex:1,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:8,padding:"8px 12px",fontSize:13,color:"#fff",fontFamily:"inherit",outline:"none"}}/>
          </div>
        </div>
      ...
      <div style={{position:"fixed",top:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:480,zIndex:10,
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div>
              <div style={{fontSize:15,fontWeight:700}}>PC 보안 점검</div>
