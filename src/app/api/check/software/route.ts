...
const TRUSTED = [
  ...
  "kakao", "naver", "hancom", "ahnlab", "안랩", "한컴", "네이버", "카카오",
  "daum", "행정안전부", "국세청", "windows", "qualcomm", "realtek", "broadcom",
]

function isTrusted(pub: string) {
  if (!pub || pub.trim() === "") return false
  const l = pub.toLowerCase()
  return TRUSTED.some(t => l.includes(t))
}
...
export async function GET() {
  try {
    ...
    // PowerShell 스크립트를 파일로 저장 후 실행 (한줄 압축 파싱 오류 방지)
